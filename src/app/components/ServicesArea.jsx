...
export default async function ServicesArea() {
  ...
  return (
    <div className="lg:w-4/5 w-11/12 mx-auto grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6 py-10">{
        data.map(service => (
            <div key={service?._id} className="p-4 bg-base-100 bg-opacity-60 shadow-md border border-gray-300 rounded-box">
                <Image className="rounded-lg w-full h-full" src={service?.img} alt={service?.title} width={314} height={208} />
            </div>
        ))
    }</div>
  )
